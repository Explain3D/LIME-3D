...
def test(model, loader, num_class=40, vote_num=1):
    ...
    for j, data in tqdm(enumerate(loader), total=len(loader)):
        ...
        if target != pred_choice:
            print('Wrongly classified! Save points')
            tmp_label = [pred_choice,target]
            np.save('confusing/data/'+str(num_err)+'_should_be'+SHAPE_NAMES[target]+'_but_'+SHAPE_NAMES[pred_choice]+'.npy',points)
            np.save('confusing/data/'+str(num_err)+'_labels.npy',tmp_label)
            num_err += 1
        num_test += 1
